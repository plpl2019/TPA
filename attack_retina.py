...
from mmdetection.mmdet.apis.inference import inference_single_attack_single_init as retina_attack_init
from mmdetection.mmdet.apis.inference import inference_single_attack_single_box as retina_attack_box
from mmdetection.mmdet.apis.inference import inference_single_attack_single_mt as retina_attack_mt
...
def make_init_mask_img(boxes_init,pred_init,labels_init,mask_layer,img_cv2_800_800, img_cv2,img_path, retina_model):
                ...
                for class_name, same_class_box in same_class.items():
                    ...
                    for i in range(divide_number):    
                        for j in range(divide_number):
                            ...
                            boxes_mask , labels_mask = retina_attack_box(img_path, retina_model, img_mask, img_mask)
...
def attack_imgs(root_path, imgs):    
    ...
    ################# retinanet init ###############
    retina_model = mmdetection_init_detector(config='./mmdetection/configs/retinanet/retinanet_r101_fpn_1x_dior.py', checkpoint='./weight/dior/retina_r101/epoch_24.pth', device='cuda:0')
    #################################################
    ...
    for ind in range(len(imgs)):
        ...
        retina_boxes,labels_pred,_ = retina_attack_init(img_path,retina_model, img_cv2_800_800,  img_cv2)
        if retina_boxes.size==0:
            continue
        with torch.no_grad():
            ##########寻找攻击区域#########
            ...
            # boxes , labels,init_det = retina_attack_init(img_path, retina_model, img_cv2_800_800, img_cv2)
            ...
            attack_map,attack_map_mean=make_init_mask_img(boxes_init,pred_init,labels_init,mask_layer,img_cv2_800_800, img_cv2,img_path, retina_model)           
            #############################
        ...
        for attack_iter in pbar:
            ...
            retina_noise, retina_boxes,labels_pred,class_loss,iou_loss = retina_attack_mt(img_path, retina_model, img_cv2_800_800, img_cv2,init_det,iou_thre) 
            noise_img = np.sign(retina_noise)
            if np.sum(np.isnan(noise_img))==original_img.size:
                break
            attack_rate =  attack_map[attack_map==1].size / attack_map.size
            output_str = imgs[ind] + '当前{}/{}'.format(imgs.index(imgs[ind]), len(imgs)) + '次数{}'.format(attack_iter)+'最初：{}'.format(ori_bbox_num)+'当前retinanet:{}'.format(len(retina_boxes))+"当前攻击比例:{}".format(attack_rate)
